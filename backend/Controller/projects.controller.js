...
export const UploadProject = async(request, response) => {
    try {
        ...
        if(!request.files.thumbnail)
            return response.status(400).json({ message: "Please upload thumbnail. It is mandatory"})

        if(!(githubURL || liveURL))
            return response.status(400).json({ message:"Github or Live URL is mandatory"})
        ...
        const thumbnail = await uploadToCloudinary(request.files.thumbnail[0].buffer)
        const projectImages = request.files.screenshots.map(file => uploadToCloudinary(file.buffer))
        ...
        await project.save();
        return response.status(201).json({ message: "Project uploaded successfully"})
...
export const GetProjectsData = async(request, response) => {
    try {
        ...
        if(!projects)
            return response.status(201).json({success:false, message: "There are no projects uploaded"})
...
export const GetProjectDetail = async(request, response) => {
    try {
        ...
        console.error("Getting error in while fetching specific project detail: ",error)
        return response.status(500).json({error: "Internal Server Error"})
    }
}